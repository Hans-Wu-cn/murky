...
import { EnhancedTable as TEnhancedTable, Loading, MessagePlugin, PrimaryTableCol, DragSortContext, TableAbnormalDragSortContext } from 'tdesign-vue-next';
...
const columns: Array<PrimaryTableCol<any>> = [
    ...
    },
    {
        ...
        cell: (h, { row, rowIndex }) => (
            <t-space>
                {
                    useAuth('tenantMenu:add', <t-link theme="primary" variant="text" hover="color" onClick={() => onAddClick(row)}>
                        {i18n.global.t('menu.button.addSub')}
                    </t-link>)
                }
                {
                    useAuth('tenantMenu:edit', <t-link theme="primary" variant="text" hover="color" onClick={() => onEditClick(row)}>
                ...
                {
                    (hasAuth('tenantMenu:remove') && !row.children?.length) ? <t-popconfirm content={() => i18n.global.t('common.label.sureDelete')} onConfirm={() => onDeleteClick(row)}>
...
const onDragSort = async (params: DragSortContext<T>) => {
    ...
    const currentRowParentId = params.target.parentTenantMenuId
    const tenantMenuIds: string[] = params.newData.filter(val => {
        return val.parentTenantMenuId === currentRowParentId
    }).map(val => val.parentTenantMenuId)
    const { code } = await dragMenu({
        parentTenantMenuId: currentRowParentId,
        tenantMenuIds
    })
    if (code === 200) {
