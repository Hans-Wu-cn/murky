...
import cn.poem.solon.admin.saas.domain.entity.table.PoemTenantGroupMenuTableDef;
import cn.poem.solon.admin.saas.domain.entity.table.PoemTenantMenuTableDef;
import cn.poem.solon.admin.saas.domain.entity.table.PoemTenantPermissionGroupTableDef;
...
public interface PoemTenantMenuMapper extends BaseMapper<PoemTenantMenu> {
    ...
    default List<PoemTenantMenu> selectByListByIds(List<Long> saasMenuIds) {

        return this.selectListByQuery(
                QueryWrapper.create()
                        .select().from(POEM_TENANT_MENU)
                        .where(POEM_TENANT_MENU.TENANT_MENU_ID.in(saasMenuIds))
                        .orderBy(POEM_TENANT_MENU.SORT.asc(), POEM_TENANT_MENU.LABEL.asc())
        );
    }
