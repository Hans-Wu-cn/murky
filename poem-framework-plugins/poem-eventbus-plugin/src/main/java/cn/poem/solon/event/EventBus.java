...
public class EventBus {
    ...
    //订阅管道
    private static Map<Class<?>, PoemEventListenPipeline<?>> sPipeline = new HashMap<>();
     ...
     */
    public synchronized static <T> void subscribe(Class<T> eventType, PoemEventListener<T> listener) {
     ...
     * @param listener  事件监听者
     */
    public synchronized static <T> void subscribe(Class<T> eventType, int index, PoemEventListener<T> listener) {
     ...
     * @param eventType 事件类型
     */
    private static <T> PoemEventListenPipeline<T> pipelineDo(Class<T> eventType) {
        PoemEventListenPipeline<T> pipeline = (PoemEventListenPipeline<T>) sPipeline.get(eventType);

        if (pipeline == null) {
            pipeline = new PoemEventListenPipeline<>();
    ...
    private static <T> void registerDo(Class<T> eventType, PoemEventListener<T> listener) {
     ...
     * @param listener 事件监听者
     */
    public synchronized static <T> void unsubscribe(org.noear.solon.core.event.EventListener<T> listener) {
        Class<?>[] ets = GenericUtil.resolveTypeArguments(listener.getClass(), org.noear.solon.core.event.EventListener.class);
    ...
    static class HH {
        protected Class<?> t;
        protected PoemEventListener l;

        public HH(Class<?> type, PoemEventListener listener) {
            this.t = type;
            this.l = listener;
        }
    }
